...
%         sigmaT_d = imresize(sigmaT,1/windowsize,'box');
        ...
        if ifDrawFFT == 0
            ...
            imagesc(sigmaT_d(:,1:size(sigmaT_d,2)/tile));colormap(copper);
            axis off
            axis image
...
%                 [fft_log_NN, fft_window_list, fft_Ratio_list] = computeFFT(sigmaT_d_cube);
                ...
                [fft_log_NN, fft_window_list, fft_Ratio_list] = computeFFT(sigmaT_d);
                subplot(4,N_downScale,flag+N_downScale*2)
                imagesc(imresize(fft_log_NN, [size(fft_log_NN,1) size(fft_log_NN,1)]));
                axis off
                axis image
                title(['FFT'])

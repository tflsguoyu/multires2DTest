function [output_reflection,output_reflection_stderr,output_insideVis] ...
    = computeScattering(hw_origin,hw_resize,albedo,NoSamples,receiptorSize,platform)
    ...
    % MATLAB 
        scatter(sigmaT_d_filename,albedo,NoSamples,...
            h_sigmaT_d,w_sigmaT_d,h_region,w_region,receiptorSize);
    end
    
    if strcmp(platform,'Windows_C')
    % C++ windows
        system(['scatter.exe ' sigmaT_d_filename ' ' num2str(albedo) ' '...
    ...
    % C++ windows
        system(['scatter_nextEvent.exe ' sigmaT_d_filename ' ' num2str(albedo) ' '...
            num2str(NoSamples) ' ' num2str(h_sigmaT_d) ' ' num2str(w_sigmaT_d) ' '...
            num2str(h_region) ' ' num2str(w_region) ' ' num2str(receiptorSize)]);
    end

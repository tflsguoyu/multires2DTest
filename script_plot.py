...
filename = 'output/binary8bit_0.95_100'

output = np.loadtxt(filename+'.csv', delimiter=',');
fftRatio = np.loadtxt(filename+'_freq.csv', delimiter=',');

trueAlbedoScale = output[:,16-2]    
originRefl = output[:,14-2]
downsampleRefl = output[:,15-2]
...
plt.plot(x,refl_vs_freq[:,4]/100,'r-',label='residual mean L2 norm')
